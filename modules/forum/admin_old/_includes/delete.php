...
$req = App::db()->query("SELECT * FROM `forum__` WHERE `id` = " . $id . " AND (`type` = 'f' OR `type` = 'r')");
if ($req->rowCount()) {
    $res = $req->fetch();
    ...
    $total = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `refid` = " . $id . " AND (`type` = 'f' OR `type` = 'r' OR `type` = 't')")->fetchColumn();
    if ($total) {
        if ($res['type'] == 'f') {
            ...
            if (isset($_POST['submit'])) {
                ...
                $check = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `id` = '$category' AND `type` = 'f'")->fetchColumn();
                if (!$check) {
                    echo __('error_wrong_data');
                    exit;
                }
                // Вычисляем правила сортировки и перемещаем разделы
                $sort = App::db()->query("SELECT * FROM `forum__` WHERE `refid` = '$category' AND `type` ='r' ORDER BY `realid` DESC")->fetch();
                $sortnum = !empty($sort['realid']) && $sort['realid'] > 0 ? $sort['realid'] + 1 : 1;
                $req_c = App::db()->query("SELECT * FROM `forum__` WHERE `refid` = " . $id . " AND `type` = 'r'");
                while ($res_c = $req_c->fetch()) {
                    App::db()->exec("UPDATE `forum__` SET `refid` = '" . $category . "', `realid` = '$sortnum' WHERE `id` = '" . $res_c['id'] . "'");
                    ++$sortnum;
                }
                // Перемещаем файлы в выбранную категорию
                App::db()->exec("UPDATE `forum__files` SET `cat` = '" . $category . "' WHERE `cat` = '" . $res['refid'] . "'");
                ...
                $req_c = App::db()->query("SELECT * FROM `forum__` WHERE `type` = 'f' AND `id` != " . $id . " ORDER BY `realid` ASC");
                while ($res_c = $req_c->fetch()) {
            ...
            if (isset($_POST['submit'])) {
                ...
                $check = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `id` = '$subcat' AND `type` = 'r'")->fetchColumn();
                if (!$check) {
                    echo __('error_wrong_data') . ' <a href="index.php?act=forum">' . __('forum_management') . '</a>';
                    exit;
                }
                App::db()->exec("UPDATE `forum__` SET `refid` = '$subcat' WHERE `refid` = " . $id);
                App::db()->exec("UPDATE `forum__files` SET `subcat` = '$subcat' WHERE `subcat` = " . $id);
                App::db()->exec("DELETE FROM `forum__` WHERE `id` = " . $id);
                ...
                // Удаляем файлы
                $req_f = App::db()->query("SELECT * FROM `forum__files` WHERE `subcat` = " . $id);
                while ($res_f = $req_f->fetch()) {
                    unlink(ROOT_PATH . 'files' . DIRECTORY_SEPARATOR . 'forum' . DIRECTORY_SEPARATOR . $res_f['filename']);
                }
                App::db()->exec("DELETE FROM `forum__files` WHERE `subcat` = " . $id);
                // Удаляем посты, голосования и метки прочтений
                $req_t = App::db()->query("SELECT `id` FROM `forum__` WHERE `refid` = " . $id . " AND `type` = 't'");
                while ($res_t = $req_t->fetch()) {
                    App::db()->exec("DELETE FROM `forum__` WHERE `refid` = '" . $res_t['id'] . "'");
                    App::db()->exec("DELETE FROM `forum__vote` WHERE `topic` = '" . $res_t['id'] . "'");
                    App::db()->exec("DELETE FROM `forum__vote_users` WHERE `topic` = '" . $res_t['id'] . "'");
                    App::db()->exec("DELETE FROM `forum__rdm` WHERE `topic_id` = '" . $res_t['id'] . "'");
                }
                // Удаляем темы
                App::db()->exec("DELETE FROM `forum__` WHERE `refid` = " . $id);
                // Удаляем раздел
                App::db()->exec("DELETE FROM `forum__` WHERE `id` = " . $id);
                // Оптимизируем таблицы
                App::db()->query("OPTIMIZE TABLE `forum__files` , `forum__rdm` , `forum__` , `forum__vote` , `forum__vote_users`");
                ...
                $req_r = App::db()->query("SELECT * FROM `forum__` WHERE `refid` = '$ref' AND `id` != " . $id . " AND `type` = 'r' ORDER BY `realid` ASC");
                ...
                $req_c = App::db()->query("SELECT * FROM `forum__` WHERE `type` = 'f' AND `id` != '$ref' ORDER BY `realid` ASC");
                while ($res_c = $req_c->fetch()) {

...
$req = App::db()->query("SELECT * FROM `forum__` WHERE `id` = " . $id);
if ($req->rowCount()) {
    $res = $req->fetch();
    if ($res['type'] == 'f' || $res['type'] == 'r') {
        if (isset($_POST['submit'])) {
            ...
            elseif ($res['type'] == 'r' && !App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `id` = '$category' AND `type` = 'f'")->fetchColumn())
                $error[] = __('error_category_select');
            if (!$name)
            ...
            if (!$error) {
                ...
                if ($res['type'] == 'r' && $category != $res['refid']) {
                    ...
                    $res_s = App::db()->query("SELECT `realid` FROM `forum__` WHERE `refid` = '$category' AND `type` = 'r' ORDER BY `realid` DESC LIMIT 1")->fetch();
                    $sort = $res_s['realid'] + 1;
                    // Меняем категорию
                    App::db()->exec("UPDATE `forum__` SET `refid` = '$category', `realid` = '$sort' WHERE `id` = " . $id);
            ...
            if ($res['type'] == 'r') {
                ...
                $req_c = App::db()->query("SELECT * FROM `forum__` WHERE `type` = 'f' ORDER BY `realid` ASC");

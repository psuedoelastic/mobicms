...
if (isset($_GET['rsort'])) {
    $sort = " AND `forum__`.`refid` = '" . abs(intval($_GET['rsort'])) . "'";
...
if (isset($_POST['deltopic'])) {
    ...
    $req = App::db()->query("SELECT `id` FROM `forum__` WHERE `type` = 't' AND `close` = '1' " . $sort);
    while ($res = $req->fetch()) {
        $req_f = App::db()->query("SELECT * FROM `forum__files` WHERE `topic` = '" . $res['id'] . "'");
        if ($req_f->rowCount()) {
            // Удаляем файлы
            while ($res_f = $req_f->fetch()) {
                unlink(ROOT_PATH . 'files' . DIRECTORY_SEPARATOR . 'forum' . DIRECTORY_SEPARATOR . $res_f['filename']);
            }
            App::db()->exec("DELETE FROM `forum__files` WHERE `topic` = '" . $res['id'] . "'");
        }
        // Удаляем посты
        App::db()->exec("DELETE FROM `forum__` WHERE `type` = 'm' AND `refid` = '" . $res['id'] . "'");
    }
    // Удаляем темы
    App::db()->exec("DELETE FROM `forum__` WHERE `type` = 't' AND `close` = '1' " . $sort);
    header('Location: ' . $uri . 'htopics/');
} else {
    $total = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type` = 't' AND `close` = '1' " . $sort)->fetchColumn();
    ...
    if ($req->rowCount()) {
        $i = 0;
        while ($res = $req->fetch()) {
            $subcat = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '" . $res['refid'] . "'")->fetch();
            $cat = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '" . $subcat['refid'] . "'")->fetch();

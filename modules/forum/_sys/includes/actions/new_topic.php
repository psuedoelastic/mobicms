...
$req_r = App::db()->query("SELECT * FROM `forum__` WHERE `id` = " . $id . " AND `type` = 'r' LIMIT 1");
if (!$req_r->rowCount()) {
...
if (isset($_POST['submit'])) {
    ...
    if (!$error) {
        ...
        App::db()->exec("INSERT INTO `forum__rdm` SET
            `topic_id` = '$rid',
            `user_id` = " . App::user()->id . ",
            `time` = '" . time() . "'
        ");
    ...
    $res_r = $req_r->fetch();
    $res_c = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '" . $res_r['refid'] . "'")->fetch();

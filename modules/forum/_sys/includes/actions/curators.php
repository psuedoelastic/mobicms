...
if (App::user()->rights >= 7) {
    ...
    $topic = App::db()->query("SELECT * FROM `forum__` WHERE `id` = " . $id . " AND `type` = 't'")->fetch();
    ...
    $req = App::db()->query("
        SELECT `forum__`.*, `user__`.`id`
        FROM `forum__`
        LEFT JOIN `user__` ON `forum__`.`user_id` = `user__`.`id`
        WHERE `forum__`.`refid` = " . $id . "
        AND `user__`.`rights` < 6
        AND `user__`.`rights` != 3
        GROUP BY `forum__`.`from`
        ORDER BY `forum__`.`from`
        ");

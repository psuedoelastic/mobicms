...
if (App::user()->rights == 3 || App::user()->rights >= 6) {
    ...
    $req = App::db()->query("SELECT * FROM `forum__` WHERE `id` = " . $id . " AND `type` = 't'");
    if (!$req->rowCount()) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($del == 2 && App::user()->rights == 9) {
            ...
            -----------------------------------------------------------------
            */
            $req1 = App::db()->query("SELECT * FROM `forum__files` WHERE `topic` = " . $id);
            if ($req1->rowCount()) {
                while ($res1 = $req1->fetch()) {
                    unlink(ROOT_PATH . 'files' . DIRECTORY_SEPARATOR . 'forum' . DIRECTORY_SEPARATOR . $res1['filename']);
                }
                App::db()->exec("DELETE FROM `forum__files` WHERE `topic` = " . $id);
                App::db()->query("OPTIMIZE TABLE `forum__files`");
            }
            ...
            */
            ...
            App::db()->exec("UPDATE `forum__files` SET `del` = '1' WHERE `topic` = " . $id);
        }

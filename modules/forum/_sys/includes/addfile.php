...
$req1 = App::db()->query("SELECT COUNT(*) FROM `forum__files` WHERE `post` = " . $id);
...
$page = ceil(App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `refid` = '" . $res['refid'] . "' AND `id` " . ($set_forum['upfp'] ? ">=" : "<=") . " '" . $res['id'] . "'")->fetchColumn() / App::user()->settings['page_size']);
            ...
            */
            if ($do_file) {
                ...
                if (!$error) {
                    ...
                    $req2 = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '" . $res['refid'] . "'");
                    $res2 = $req2->fetch();
                    $req3 = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '" . $res2['refid'] . "'");
                    $res3 = $req3->fetch();

                    // Заносим данные в базу
                    $stmt = App::db()->prepare("
                        INSERT INTO `forum__files`

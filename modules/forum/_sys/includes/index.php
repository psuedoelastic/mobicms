...
if ($id) {
    ...
    $type = App::db()->query("SELECT * FROM `forum__` WHERE `id`= " . $id);
    ...
    if (App::user()->id && $type1['type'] == 't') {
        $req_r = App::db()->query("SELECT * FROM `forum__rdm` WHERE `topic_id` = " . $id . " AND `user_id` = " . App::user()->id . " LIMIT 1");

        if ($req_r->rowCount()) {
            $res_r = $req_r->fetch();
            if ($type1['time'] > $res_r['time']) {
                App::db()->exec("UPDATE `forum__rdm` SET `time` = '" . time() . "' WHERE `topic_id` = " . $id . " AND `user_id` = " . App::user()->id . " LIMIT 1");
            }
        } else {
            App::db()->exec("INSERT INTO `forum__rdm` SET `topic_id` = " . $id . ", `user_id` = " . App::user()->id . ", `time` = '" . time() . "'");
        }
    }
    ...
    while ($parent != '0' && $res != false) {
        $res = App::db()->query("SELECT * FROM `forum__` WHERE `id` = '$parent' LIMIT 1")->fetch();
    ...
    if ($type1['type'] == 'f') {
        $count = App::db()->query("SELECT COUNT(*) FROM `forum__files` WHERE `cat` = " . $id . $sql)->fetchColumn();
        if ($count > 0) {
            $filelink = '<a href="' . $uri . '?act=files&amp;c=' . $id . '">' . __('files_category') . '</a>';
        }
    } elseif ($type1['type'] == 'r') {
        $count = App::db()->query("SELECT COUNT(*) FROM `forum__files` WHERE `subcat` = " . $id . $sql)->fetchColumn();
        if ($count > 0) {
            $filelink = '<a href="' . $uri . '?act=files&amp;s=' . $id . '">' . __('files_section') . '</a>';
        }
    } elseif ($type1['type'] == 't') {
        $count = App::db()->query("SELECT COUNT(*) FROM `forum__files` WHERE `topic` = " . $id . $sql)->fetchColumn();
        if ($count > 0) {
        ...
        */
        switch ($type1['type']) {
            case 'f':
                 ...
                 */
                $req = App::db()->query("SELECT `id`, `text`, `soft` FROM `forum__` WHERE `type`='r' AND `refid` = " . $id . " ORDER BY `realid`");
                App::view()->total = $req->rowCount();

                if (App::view()->total > 0) {
                    for ($i = 0; $res = $req->fetch(); ++$i) {
                        $res['count'] = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type` = 't' AND `refid` = '" . $res['id'] . "'")->fetchColumn();
                        App::view()->list[$i] = $res;
                    }
                }
            ...
            case 'r':
                 ...
                 * Список топиков
                 */
                App::view()->total = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type`='t' AND `refid` = " . $id . (App::user()->rights >= 7 ? '' : " AND `close`!='1'"))->fetchColumn();
                ...
                if (App::view()->total > 0) {
                    $req = App::db()->query("SELECT * FROM `forum__` WHERE `type`='t'" . (App::user()->rights >= 7 ? '' : " AND `close`!='1'") . " AND `refid` = " . $id . " ORDER BY `vip` DESC, `time` DESC " . App::db()->pagination());

                    for ($i = 0; $res = $req->fetch(); ++$i) {
                        $nam = App::db()->query("SELECT `from` FROM `forum__` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" . $res['id'] . "' ORDER BY `time` DESC LIMIT 1")->fetch();
                        $res['countmsg'] = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type`='m' AND `refid`='" . $res['id'] . "'" . (App::user()->rights >= 7 ? '' : " AND `close` != '1'"))->fetchColumn();
                        $cpg = ceil($res['countmsg'] / App::user()->settings['page_size']);

                        if (App::user()->id) {
                            $res['unread'] = App::db()->query("SELECT COUNT(*) FROM `forum__rdm` WHERE `time` >= '" . $res['time'] . "' AND `topic_id` = '" . $res['id'] . "' AND `user_id` = " . App::user()->id)->fetchColumn();
                        } else {
                            $res['unread'] = 1;
                        }
            ...
            case 't':
                ...
                // Счетчик постов темы
                $colmes = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type`='m'$sql AND `refid` = " . $id . (App::user()->rights >= 7 ? '' : " AND `close` != '1'"))->fetchColumn();
...
//                    $vote_user = App::db()->query("SELECT COUNT(*) FROM `forum__vote_users` WHERE `user` = " . App::user()->id . " AND `topic` = " . App::vars()->id)->fetchColumn();
//                    $topic_vote = App::db()->query("SELECT `name`, `time`, `count` FROM `forum__vote` WHERE `type`='1' AND `topic` = " . App::vars()->id . " LIMIT 1")->fetch();
//                    echo '<div  class="gmenu"><b>' . htmlspecialchars($topic_vote['name']) . '</b><br />';
//                    $vote_result = App::db()->query("SELECT `id`, `name`, `count` FROM `forum__vote` WHERE `type`='2' AND `topic` = " . App::vars()->id . " ORDER BY `id` ASC");
...
//                    $postres = App::db()->query("SELECT `forum__`.*, `user__`.`sex`, `user__`.`rights`, `user__`.`last_visit`, `user__`.`status`, `user__`.`datereg`
//                    FROM `forum__` LEFT JOIN `user__` ON `forum__`.`user_id` = `user__`.`id`
//                    WHERE `forum__`.`type` = 'm' AND `forum__`.`refid` = " . App::vars()->id . (App::user()->rights >= 7 ? "" : " AND `forum__`.`close` != '1'") . "
//                    ORDER BY `forum__`.`id` LIMIT 1")->fetch();
...
//                }
                ...
                ////////////////////////////////////////////////////////////
                $req = App::db()->query("
                    SELECT `forum__`.*, `user__`.`sex`, `user__`.`rights`, `user__`.`last_visit`, `user__`.`status`, `user__`.`join_date`, `user__`.`avatar`
                    FROM `forum__`
                    LEFT JOIN `user__` ON `forum__`.`user_id` = `user__`.`id`
                    WHERE `forum__`.`type` = 'm'
                    AND `forum__`.`refid` = " . $id . (App::user()->rights >= 7 ? "" : "
                    AND `forum__`.`close` != '1'") . "$sql ORDER BY `forum__`.`id` $order " .
                    App::db()->pagination());
...
//                }
                ...
                for ($i = 1; $res = $req->fetch(); ++$i) {
                    ...
                    $freq = App::db()->query("SELECT * FROM `forum__files` WHERE `post` = '" . $res['id'] . "'");
                    if ($freq->rowCount() > 0) {
                        $fres = $freq->fetch();
         ...
         */
        $req = App::db()->query("SELECT `id`, `text`, `soft` FROM `forum__` WHERE `type` = 'f' ORDER BY `realid`");
        App::view()->total = $req->rowCount();

        if (App::view()->total > 0) {
            for ($i = 0; $res = $req->fetch(); ++$i) {
                $res['count'] = App::db()->query("SELECT COUNT(*) FROM `forum__` WHERE `type` = 'r' AND `refid` = '" . $res['id'] . "'")->fetchColumn();
                App::view()->list[$i] = $res;
            }
        }

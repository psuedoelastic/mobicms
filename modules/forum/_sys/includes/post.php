...
// Запрос сообщения
$req = App::db()->query("SELECT `forum__`.*, `user__`.`sex`, `user__`.`rights`, `user__`.`last_visit`, `user__`.`status`, `user__`.`join_date`
FROM `forum__` LEFT JOIN `user__` ON `forum__`.`user_id` = `user__`.`id`
WHERE `forum__`.`type` = 'm' AND `forum__`.`id` = " . $id . (App::user()->rights >= 7 ? "" : " AND `forum__`.`close` != '1'") . " LIMIT 1");
$res = $req->fetch();

// Запрос темы
$them = App::db()->query("SELECT * FROM `forum__` WHERE `type` = 't' AND `id` = '" . $res['refid'] . "'")->fetch();

...
if (isset($actions[$act]) && is_file(__DIR__ . DS . '_sys' . DS . 'includes' . $actions[$act] . $act . '.php')) {
    ...
    */
    $notice = false;
    if ($id) {
        $cat = App::db()->query("SELECT * FROM `" . TP . "download__category` WHERE `id` = " . $id);
        ...
        $total_new = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `type` = '2'  AND `time` > $old AND `dir` LIKE '" . ($res_down_cat['dir']) . "%'")->fetchColumn();
        if ($total_new)
        ...
        $total_new = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `type` = '2'  AND `time` > $old")->fetchColumn();
        if ($total_new) {
            $notice = '<a href="' . $url . '?act=new_files&amp;id=' . $id . '">' . __('new_files') . '</a> (' . $total_new . ')<br />';
        }
    }
    if (App::user()->rights == 4 || App::user()->rights >= 6) {
        $mod_files = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `type` = '3'")->fetchColumn();
        if ($mod_files > 0) {
    ...
    */
    ...
    -----------------------------------------------------------------
    */
    $total_cat = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__category` WHERE `refid` = '" . $id . "'")->fetchColumn();
    $total_files = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `refid` = '" . $id . "' AND `type` = 2")->fetchColumn();
    ...
    if ($sum_total) {
        if ($total_cat > 0) {
            ...
            $req_down = App::db()->query("SELECT * FROM `" . TP . "download__category` WHERE `refid` = '" . $id . "' ORDER BY `sort` ASC ");
            $i = 0;
        ...
        if ($total_files > 0) {
            ...
            /*
              ...
              -----------------------------------------------------------------
              */
            $req_down = App::db()->query("SELECT * FROM `" . TP . "download__files` WHERE `refid` = '" . $id . "' AND `type` < 3 ORDER BY `type` ASC $sql_sort " . App::db()->pagination());
            $i = 0;
            while ($res_down = $req_down->fetch()) {

...
class Download
{
    ...
    */
    public static function displayFile($res_down = [], $rate = 0)
    {
        ...
        $out .= '<a href="' . App::router()->getUri(1) . '?act=view&amp;id=' . $res_down['id'] . '">' . htmlspecialchars($res_down['rus_name']) . '</a> (' . $res_down['field'] . ')';
        ...
        if (App::cfg()->sys->acl_downloads_comm || App::user()->rights >= 7) {
            $sub .= '<a href="' . App::router()->getUri(1) . '?act=comments&amp;id=' . $res_down['id'] . '">' . __('comments') . '</a> (' . $res_down['total'] . ')';
        }
        if ($sub) {
    ...
    */
    ...
    {
        if (!$value) return htmlspecialchars(iconv('windows-1251', 'UTF-8', $name));
    ...
    */
    public static function downloadLlink($array = [])
    {
        ...
        $out .= '</td><td><a href="' . $url . '?act=load_file&amp;id=' . $id . $morelink . '">' . $array['res']['text'] . '</a> (' . Download::displayFileSize((isset($array['res']['size']) ? $array['res']['size'] : filesize($array['res']['dir'] . '/' . $array['res']['name']))) . ')';
        if ($array['res']['time'] > $old) {
            $out .= ' <span class="red">(NEW)</span>';
        }
        $out .= '<div class="sub">' . __('file_time') . ': ' . Functions::displayDate($array['res']['time']);
        if ($array['format'] == 'jar') {
            $out .= ', <a href="' . $url . '?act=jad_file&amp;id=' . $id . $morelink . '">JAD</a>';
        } elseif ($array['format'] == 'txt') {
            $out .= ', <a href="' . $url . '?act=txt_in_zip&amp;id=' . $id . $morelink . '">ZIP</a> / <a href="' . $url . '?act=txt_in_jar&amp;id=' . $id . $morelink . '">JAR</a>';
        } else {
            if ($array['format'] == 'zip') {
                $out .= ', <a href="' . $url . '?act=open_zip&amp;id=' . $id . $morelink . '">' . __('open_archive') . '</a>';
            }
        }
        $out .= '</div></td></tr></table>';
    ...
    */
    ...
    {
        $url = App::router()->getUri(1);
        $category = ['<a href="' . $url . '"><b>' . __('download_title') . '</b></a>'];
        if ($array['refid']) {
            $sql = [];
            ...
            if ($sql) {
                $req_cat = App::db()->query("SELECT * FROM `" . TP . "download__category` WHERE `dir` IN ('" . implode("','", $sql) . "') ORDER BY `id` ASC");
                while ($res_cat = $req_cat->fetch()) {
                    $category[] = '<a href="' . $url . '?id=' . $res_cat['id'] . '">' . htmlspecialchars($res_cat['rus_name']) . '</a>';
                }
            }
        }
        if (isset($array['name'])) $category[] = htmlspecialchars($array['name']);

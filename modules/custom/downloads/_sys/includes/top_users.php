...
$req = App::db()->query("SELECT * FROM `" . TP . "download__files` WHERE `user_id` > 0 GROUP BY `user_id` ORDER BY COUNT(`user_id`)");
$total = $req->rowCount();
...
$i = 0;
if ($total) {
    $req_down = App::db()->query("SELECT *, COUNT(`user_id`) AS `count` FROM `" . TP . "download__files` WHERE `user_id` > 0 GROUP BY `user_id` ORDER BY `count` DESC " . App::db()->pagination());
    while ($res_down = $req_down->fetch()) {
        $user = App::db()->query("SELECT * FROM `" . TP . "user__` WHERE `id`=" . $res_down['user_id'])->fetch();

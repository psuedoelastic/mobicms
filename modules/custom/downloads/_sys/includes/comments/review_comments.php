...
$total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__comments`")->fetchColumn();
if ($total) {
    $req = App::db()->query("SELECT `" . TP . "download__comments`.*, `" . TP . "download__comments`.`id` AS `cid`, `" . TP . "user__`.`rights`, `" . TP . "user__`.`last_visit`, `" . TP . "user__`.`sex`, `" . TP . "user__`.`status`, `" . TP . "user__`.`join_date`, `" . TP . "user__`.`id`, `" . TP . "download__files`.`rus_name`
	FROM `" . TP . "download__comments` LEFT JOIN `" . TP . "user__` ON `" . TP . "download__comments`.`user_id` = `" . TP . "user__`.`id` LEFT JOIN `" . TP . "download__files` ON `" . TP . "download__comments`.`sub_id` = `" . TP . "download__files`.`id` ORDER BY `" . TP . "download__comments`.`time` DESC " . App::db()->pagination());
    $i = 0;
    /*
    -----------------------------------------------------------------
    Навигация

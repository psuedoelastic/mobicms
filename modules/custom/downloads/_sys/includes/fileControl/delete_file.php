...
if (App::user()->rights == 4 || App::user()->rights >= 6) {
    if (isset($_GET['yes'])) {
        ...
        @unlink(ROOTPATH . 'files/download/java_icons/' . $id . '.png');
        $req_file_more = App::db()->query("SELECT * FROM `" . TP . "download__more` WHERE `refid` = " . $id);
        if ($req_file_more->rowCount()) {
            while ($res_file_more = $req_file_more->fetch()) {
                ...
                @unlink(ROOTPATH . 'files/download/java_icons/' . $res_file_more['id'] . '_' . $id . '.png');
            }
            App::db()->exec("DELETE FROM `" . TP . "download__more` WHERE `refid` = " . $id);
        }
        App::db()->exec("DELETE FROM `" . TP . "download__bookmark` WHERE `file_id` = " . $id);
        App::db()->exec("DELETE FROM `" . TP . "download__comments` WHERE `sub_id` = " . $id);
        @unlink($res_down['dir'] . '/' . $res_down['name']);
        $dirid = $res_down['refid'];
        $sql = '';
        $i = 0;
        while ($dirid != '0' && $dirid != "") {
            $res = App::db()->query("SELECT `refid` FROM `" . TP . "download__category` WHERE `id` = '$dirid' LIMIT 1")->fetch();
            if ($i) $sql .= ' OR ';
            $sql .= '`id` = \'' . $dirid . '\'';
            $dirid = $res['refid'];
            ++$i;
        ...
        App::db()->exec("DELETE FROM `" . TP . "download__files` WHERE `id` = " . $id);
        App::db()->query("OPTIMIZE TABLE `" . TP . "download__files`");

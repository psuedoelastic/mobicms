...
if ($search && !$error) {
    ...
    */
    ...
    $total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `type` = '2'  AND $sql")->fetchColumn();
    ...
    if ($total) {
        $req_down = App::db()->query("SELECT * FROM `" . TP . "download__files` WHERE `type` = '2'  AND $sql ORDER BY `rus_name` ASC " . App::db()->pagination());
        $i = 0;

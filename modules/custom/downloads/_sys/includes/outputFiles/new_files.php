...
$sql_down = '';
if (App::vars()->id) {
    $cat = App::db()->query("SELECT * FROM `" . TP . "download__category` WHERE `id` = '" . App::vars()->id . "' LIMIT 1");
...
$total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__files` WHERE `type` = '2'  AND `time` > $sql_down")->fetchColumn();
/*
-----------------------------------------------------------------
Навигация
-----------------------------------------------------------------
...
if ($total) {
    $i = 0;
    $req_down = App::db()->query("SELECT * FROM `" . TP . "download__files` WHERE `type` = '2'  AND `time` > $old $sql_down ORDER BY `time` DESC " . App::db()->pagination());

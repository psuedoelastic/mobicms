...
$total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "download__bookmark` WHERE `user_id` = " . App::user()->id)->fetchColumn();
/*
-----------------------------------------------------------------
Навигация
-----------------------------------------------------------------
...
$i = 0;
if ($total) {
    $req_down = App::db()->query("SELECT `" . TP . "download__files`.*, `" . TP . "download__bookmark`.`id` AS `bid`
    FROM `" . TP . "download__files` LEFT JOIN `" . TP . "download__bookmark` ON `" . TP . "download__files`.`id` = `" . TP . "download__bookmark`.`file_id`
    WHERE `" . TP . "download__bookmark`.`user_id`=" . App::user()->id . " ORDER BY `" . TP . "download__files`.`time` DESC " . App::db()->pagination());

        ...
        if ($search && !$error) {
            ...
            */
            ...
            if ($search_t) {
                ...
                $stmt = App::db()->prepare("
                    SELECT COUNT(*) FROM `" . TP . "forum__`, `" . TP . "forum__` AS `forum2`
                    WHERE MATCH (`" . TP . "forum__`.`text`) AGAINST (? IN BOOLEAN MODE)
                    AND `" . TP . "forum__`.`type` = 'm'
					AND `forum2`.`id` = `" . TP . "forum__`.`refid`
					" . (App::user()->rights >= 7 ? "" : "AND `forum2`.`close` != '1' AND `" . TP . "forum__`.`close` != '1'
				"));
            ...
            if ($total) {
                ...
                if ($search_t) {
                    ...
                    // Поиск только в тексте
                    $stmt = App::db()->prepare("
                        SELECT `" . TP . "forum__`.*, `forum2`.`id` as `id2`, `forum2`.`text` as `text2`,
						MATCH (`" . TP . "forum__`.`text`) AGAINST (:query IN BOOLEAN MODE) as `rel`
                        FROM `" . TP . "forum__`, `" . TP . "forum__` as `forum2`
                        WHERE MATCH (`" . TP . "forum__`.`text`) AGAINST (:query IN BOOLEAN MODE)
                        AND `" . TP . "forum__`.`type` = 'm'
						AND `forum2`.`id` = `" . TP . "forum__`.`refid`
						" . (App::user()->rights >= 7 ? "" : "AND `forum2`.`close` != '1' AND `" . TP . "forum__`.`close` != '1'") . "
                        ORDER BY `rel` DESC
                ...
                while ($res = $stmt->fetch()) {
                    ...
                    if ($search_t) {
                        // Поиск в названиях тем
                        $req_p = App::db()->query("SELECT `text` FROM `" . TP . "forum__` WHERE `refid` = " . $res['id'] .

...
$uri = '';
if (isset($_GET['tsort'])) {
    $sort = " AND `" . TP . "forum__`.`refid` = '" . abs(intval($_GET['tsort'])) . "'";
...
if (isset($_POST['delpost'])) {
    ...
    $req = App::db()->query("SELECT `id` FROM `" . TP . "forum__` WHERE `type` = 'm' AND `close` = '1' " . $sort);
    while ($res = $req->fetch()) {
        $req_f = App::db()->query("SELECT * FROM `" . TP . "forum__files` WHERE `post` = '" . $res['id'] . "' LIMIT 1");
        if ($req_f->rowCount()) {
            $res_f = $req_f->fetch();
            // Удаляем файлы
            unlink(ROOT_PATH . 'files' . DIRECTORY_SEPARATOR . 'forum' . DIRECTORY_SEPARATOR . $res_f['filename']);
            App::db()->exec("DELETE FROM `" . TP . "forum__files` WHERE `post` = '" . $res['id'] . "' LIMIT 1");
        }
    }
    // Удаляем посты
    App::db()->exec("DELETE FROM `" . TP . "forum__` WHERE `type` = 'm' AND `close` = '1' " . $sort);
    header('Location: ' . $uri . 'hposts/');
} else {
    $total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `type` = 'm' AND `close` = '1' " . $sort)->fetchColumn();
    ...
    if ($req->rowCount()) {
        ...
        while ($res = $req->fetch()) {
            ...
            $page = ceil(App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `refid` = '" . $res['refid'] . "' AND `id` " . ($set_forum['upfp'] ? ">=" : "<=") . " '" . $res['fid'] . "'")->fetchColumn() / App::user()->settings['page_size']);
            $text = mb_substr($res['text'], 0, 500);
            $text = htmlspecialchars($text);
            $text = preg_replace('#\[c\](.*?)\[/c\]#si', '<div class="quote">\1</div>', $text);
            $theme = App::db()->query("SELECT `id`, `text` FROM `" . TP . "forum__` WHERE `id` = '" . $res['refid'] . "'")->fetch();

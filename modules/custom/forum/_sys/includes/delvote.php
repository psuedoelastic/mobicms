...
if (App::user()->rights == 3 || App::user()->rights >= 6) {
    ...
    if (isset($_GET['yes'])) {
        App::db()->exec("DELETE FROM `" . TP . "forum__vote` WHERE `topic` = " . $id);
        App::db()->exec("DELETE FROM `" . TP . "forum__vote_users` WHERE `topic` = " . $id);
        App::db()->exec("UPDATE `" . TP . "forum__` SET  `realid` = '0'  WHERE `id` = " . $id);

...
$req = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `id` = " . $id . " AND `type` = 'm' " . (App::user()->rights >= 7 ? "" : " AND `close` != '1'"));
if ($req->rowCount()) {
    /*
    -----------------------------------------------------------------
    ...
    */
    ...
    if (App::user()->rights < 6 && App::user()->rights != 3 && App::user()->id) {
        $topic = App::db()->query("SELECT `curators` FROM `" . TP . "forum__` WHERE `id` = " . $res['refid'])->fetch();
        $curators = !empty($topic['curators']) ? unserialize($topic['curators']) : [];
        if (array_key_exists(App::user()->id, $curators)) {
            App::user()->rights = 3;
        }
    }
    $page = ceil(App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `refid` = '" . $res['refid'] . "' AND `id` " . ($set_forum['upfp'] ? ">= " : "<= ") . $id . (App::user()->rights < 7 ? " AND `close` != '1'" : ''))->fetchColumn() / App::user()->settings['page_size']);
    $posts = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `refid` = '" . $res['refid'] . "' AND `close` != '1'")->fetchColumn();
    ...
    if (App::user()->rights == 3 || App::user()->rights >= 6) {
        ...
        if ($res['user_id'] != App::user()->id) {
            $req_u = App::db()->query("SELECT * FROM `" . TP . "user__` WHERE `id` = '" . $res['user_id'] . "'");
            if ($req_u->rowCount()) {
                $res_u = $req_u->fetch();
        ...
        if (!$error) {
            $req_m = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `refid` = '" . $res['refid'] . "' ORDER BY `id` DESC LIMIT 1");
            $res_m = $req_m->fetch();
...
if (!$error) {
    switch (App::request()->getQuery('mod', '')) {
        case 'restore':
            ...
            -----------------------------------------------------------------
            */
            $req_u = App::db()->query("SELECT `count_forum__` FROM `" . TP . "user__` WHERE `id` = '" . $res['user_id'] . "'");
            if ($req_u->rowCount()) {
                // Добавляем один балл к счетчику постов юзера
                $res_u = $req_u->fetch();
                App::db()->exec("UPDATE `" . TP . "user__` SET `count_forum__` = '" . ($res_u['count_forum'] + 1) . "' WHERE `id` = '" . $res['user_id'] . "'");
            }
            App::db()->exec("UPDATE `" . TP . "forum__` SET `close` = '0', `close_who` = " . $nick . " WHERE `id` = " . $id);
            $req_f = App::db()->query("SELECT * FROM `" . TP . "forum__files` WHERE `post` = " . $id . " LIMIT 1");
            if ($req_f->rowCount()) {
                App::db()->exec("UPDATE `" . TP . "forum__files` SET `del` = '0' WHERE `post` = " . $id . " LIMIT 1");
            }
            ...
            */
            if ($res['close'] != 1) {
                ...
                if ($req_u->rowCount()) {
                    ...
                    App::db()->exec("UPDATE `" . TP . "user__` SET `count_forum__` = '" . $postforum . "' WHERE `id` = '" . $res['user_id'] . "'");
                }
            }
            if (App::user()->rights == 9 && !isset($_GET['hide'])) {
                ...
                if ($req_f->rowCount()) {
                    ...
                    App::db()->exec("DELETE FROM `" . TP . "forum__files` WHERE `post` = " . $id . " LIMIT 1");
                }
                ...
                if ($req_f->rowCount()) {
                    ...
                    App::db()->exec("UPDATE `" . TP . "forum__files` SET `del` = '1' WHERE `post` = " . $id . " LIMIT 1");
                }
                if ($posts == 1) {
                    // Если это был последний пост темы, то скрываем саму тему
                    $res_l = App::db()->query("SELECT `refid` FROM `" . TP . "forum__` WHERE `id` = '" . $res['refid'] . "'")->fetch();
                    App::db()->exec("UPDATE `" . TP . "forum__` SET `close` = '1', `close_who` = " . $nick . " WHERE `id` = '" . $res['refid'] . "' AND `type` = 't'");
            ...
            */
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($msg && !isset($_POST['submit'])) {
                    $user = App::db()->query("SELECT * FROM `" . TP . "user__` WHERE `id` = '" . $res['user_id'] . "' LIMIT 1")->fetch();

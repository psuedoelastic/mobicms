...
if ($do || isset($_GET['new'])) {
    ...
    -----------------------------------------------------------------
    */
    $total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__files` WHERE " . (isset($_GET['new'])
    ...
    if ($total > 0) {
        ...
        $req = App::db()->query("SELECT `" . TP . "forum__files`.*, `" . TP . "forum__`.`user_id`, `" . TP . "forum__`.`text`, `topicname`.`text` AS `topicname`
            FROM `" . TP . "forum__files`
            LEFT JOIN `" . TP . "forum__` ON `" . TP . "forum__files`.`post` = `" . TP . "forum__`.`id`
            LEFT JOIN `" . TP . "forum__` AS `topicname` ON `" . TP . "forum__files`.`topic` = `topicname`.`id`
            WHERE " . (isset($_GET['new']) ? " `" . TP . "forum__files`.`time` > '$new'" : " `filetype` = '$do'") . (App::user()->rights >= 7 ? '' : " AND `del` != '1'") . $sql .
            "ORDER BY `time` DESC " . App::db()->pagination()
        );
        $i = 0;
        while ($res = $req->fetch()) {
            $req_u = App::db()->query("SELECT `id`, `nickname`, `sex`, `rights`, `last_visit`, `status`, `join_date`, `ip`, `user_agent` FROM `" . TP . "user__` WHERE `id` = '" . $res['user_id'] . "'");
            $res_u = $req_u->fetch();
            ...
            $page = ceil(App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `refid` = '" . $res['topic'] . "' AND `id` " . ($set_forum['upfp']
    ...
    -----------------------------------------------------------------
    */
    $countnew = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__files` WHERE `time` > '$new'" . (App::user()->rights >= 7 ? '' : " AND `del` != '1'") . $sql)->fetchColumn();
    ...
    $total = 0;
    for ($i = 1; $i < 10; $i++) {
        $count = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__files` WHERE `filetype` = '$i'" . (App::user()->rights >= 7

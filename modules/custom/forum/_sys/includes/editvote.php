...
if (App::user()->rights == 3 || App::user()->rights >= 6) {
    ...
    if (isset($_GET['delvote']) && !empty($_GET['vote'])) {
        ...
        $totalvote = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote` WHERE `type` = '2' AND `id` = '$vote' AND `topic` = " . $id)->fetchColumn();
        $countvote = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote` WHERE `type` = '2' AND `topic` = " . $id)->fetchColumn();
        if ($countvote <= 2)
            header('location: ?act=editvote&id=' . $id . '');
        if ($totalvote != 0) {
            if (isset($_GET['yes'])) {
                App::db()->exec("DELETE FROM `" . TP . "forum__vote` WHERE `id` = '$vote'");
                $countus = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote_users` WHERE `vote` = '$vote' AND `topic` = " . $id)->fetchColumn();
                $topic_vote = App::db()->query("SELECT `count` FROM `" . TP . "forum__vote` WHERE `type` = '1' AND `topic` = " . $id . " LIMIT 1")->fetch();
                $totalcount = $topic_vote['count'] - $countus;
                App::db()->exec("UPDATE `" . TP . "forum__vote` SET  `count` = '$totalcount'   WHERE `type` = '1' AND `topic` = " . $id);
                App::db()->exec("DELETE FROM `" . TP . "forum__vote_users` WHERE `vote` = '$vote'");
                header('location: ?act=editvote&id=' . $id . '');
            } else {
        ...
        if (!empty($vote_name)) {
            App::db()->exec("UPDATE `" . TP . "forum__vote` SET  `name` = " . $vote_name . "  WHERE `topic` = " . $id . " AND `type` = '1'");
        }
        $vote_result = App::db()->query("SELECT `id` FROM `" . TP . "forum__vote` WHERE `type`='2' AND `topic` = " . $id);
        while ($vote = $vote_result->fetch()) {
            if (!empty($_POST[$vote['id'] . 'vote'])) {
                $text = App::db()->quote(mb_substr(trim($_POST[$vote['id'] . 'vote']), 0, 30));
                App::db()->exec("UPDATE `" . TP . "forum__vote` SET  `name` = " . $text . "  WHERE `id` = '" . $vote['id'] . "'");
            }
        }
        $countvote = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote` WHERE `type`='2' AND `topic` = " . $id)->fetchColumn();
        for ($vote = $countvote; $vote < 20; $vote++) {
            if (!empty($_POST[$vote])) {
                ...
                App::db()->exec("INSERT INTO `" . TP . "forum__vote` SET `name` = " . $text . ",  `type` = '2', `topic` = " . $id);
            }
        }
        ...
        */
        $countvote = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote` WHERE `type` = '2' AND `topic` = " . $id)->fetchColumn();
        $topic_vote = App::db()->query("SELECT `name` FROM `" . TP . "forum__vote` WHERE `type` = '1' AND `topic` = " . $id . " LIMIT 1")->fetch();
        echo '<div class="phdr"><a href="' . $url . '?id=' . $id . '"><b>' . __('forum') . '</b></a> | ' . __('edit_vote') . '</div>' .
            ...
            '</p></div>' .
            '<div class="menu"><p>';
        $vote_result = App::db()->query("SELECT `id`, `name` FROM `" . TP . "forum__vote` WHERE `type` = '2' AND `topic` = " . $id);
        $i = 0;
        while ($vote = $vote_result->fetch()) {

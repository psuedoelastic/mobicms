...
if (App::user()->rights == 3 || App::user()->rights >= 6) {
    ...
    $typ = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `id` = " . $id . " AND `type` = 't'");
    if (!$typ->rowCount()) {
        echo __('error_wrong_data');
        exit;
    }
    if (isset($_POST['submit'])) {
        ...
        $typ1 = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `id` = '$razd' AND `type` = 'r'");
        if (!$typ1->rowCount()) {
            echo __('error_wrong_data');
            exit;
        }

        App::db()->exec("UPDATE `" . TP . "forum__` SET `refid` = '$razd' WHERE `id` = " . $id);
        ...
        */
        $ms = $typ->fetch();
        if (empty($_GET['other'])) {
            $rz = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE id='" . $ms['refid'] . "'");
            $rz1 = $rz->fetch();
            $other = $rz1['refid'];
        } else {
            $other = abs(intval($_GET['other']));
        }
        $fr = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE id='" . $other . "'");
        $fr1 = $fr->fetch();
        ...
        $raz = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `refid` = '$other' AND `type` = 'r' AND `id` != '" . $ms['refid'] . "' ORDER BY `realid` ASC");
        if ($raz->rowCount()) {
        ...
        $frm = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `type` = 'f' AND `id` != '$other' ORDER BY `realid` ASC");

...
if ($topic_vote == 0) {
    ...
    $topic_vote = App::db()->query("SELECT `name`, `time`, `count` FROM `" . TP . "forum__vote` WHERE `type` = '1' AND `topic` = " . $id . " LIMIT 1")->fetch();
    echo '<div  class="phdr">' . __('voting_users') . ' &laquo;<b>' . htmlentities($topic_vote['name'], ENT_QUOTES, 'UTF-8') . '</b>&raquo;</div>';
    $total = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__vote_users` WHERE `topic`=" . $id)->fetchColumn();
    $req = App::db()->query("SELECT `" . TP . "forum__vote_users`.*, `" . TP . "user__`.`rights`, `" . TP . "user__`.`last_visit`, `" . TP . "user__`.`nickname`, `" . TP . "user__`.`sex`, `" . TP . "user__`.`status`, `" . TP . "user__`.`join_date`, `" . TP . "user__`.`id`
    FROM `" . TP . "forum__vote_users` LEFT JOIN `" . TP . "user__` ON `" . TP . "forum__vote_users`.`user` = `" . TP . "user__`.`id`
    WHERE `" . TP . "forum__vote_users`.`topic`=" . $id . " ORDER BY `" . TP . "forum__vote_users`.`id` DESC " . App::db()->pagination());

...
if (App::user()->id) {
    switch (App::request()->getQuery('act', '')) {
        case 'reset':
            ...
            -----------------------------------------------------------------
            */
            $req = App::db()->query("SELECT `" . TP . "forum__`.`id`
                FROM `" . TP . "forum__` LEFT JOIN `" . TP . "forum__rdm` ON `" . TP . "forum__`.`id` = `" . TP . "forum__rdm`.`topic_id` AND `" . TP . "forum__rdm`.`user_id` = " . App::user()->id . "
                WHERE `" . TP . "forum__`.`type`='t'
                AND `" . TP . "forum__rdm`.`topic_id` Is Null"
            );
            $stmt = App::db()->prepare("
                INSERT INTO `" . TP . "forum__rdm`
            ...
            $req = App::db()->query("SELECT `" . TP . "forum__`.`id` AS `id`
                FROM `" . TP . "forum__` LEFT JOIN `" . TP . "forum__rdm` ON `" . TP . "forum__`.`id` = `" . TP . "forum__rdm`.`topic_id` AND `" . TP . "forum__rdm`.`user_id` = " . App::user()->id . "
                WHERE `" . TP . "forum__`.`type`='t'
                AND `" . TP . "forum__`.`time` > `" . TP . "forum__rdm`.`time`"
            );
            $stmt = App::db()->prepare("
                UPDATE `" . TP . "forum__rdm` SET
                `time` = " . time() . "
                WHERE `topic_id` = ?
        ...
        case 'period':
            ...
            */
            ...
            if (App::user()->rights == 9) {
                $req = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `type`='t' AND `time` > '$vr1'");
            } else {
                $req = App::db()->query("SELECT COUNT(*) FROM `" . TP . "forum__` WHERE `type`='t' AND `time` > '$vr1' AND `close` != '1'");
            }
            ...
            if ($count > 0) {
                if (App::user()->rights == 9) {
                    $req = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `type`='t' AND `time` > '" . $vr1 . "' ORDER BY `time` DESC " . App::db()->pagination());
                } else {
                    $req = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `type`='t' AND `time` > '" . $vr1 . "' AND `close` != '1' ORDER BY `time` DESC " . App::db()->pagination());
                }
                for ($i = 0; $res = $req->fetch(); ++$i) {
                    echo $i % 2 ? '<div class="list2">' : '<div class="list1">';
                    $q3 = App::db()->query("SELECT `id`, `refid`, `text` FROM `" . TP . "forum__` WHERE `type`='r' AND `id`='" . $res['refid'] . "'");
                    $razd = $q3->fetch();
                    $q4 = App::db()->query("SELECT `text` FROM `" . TP . "forum__` WHERE `type`='f' AND `id`='" . $razd['refid'] . "'");
                    $frm = $q4->fetch();
                    $colmes = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `refid` = '" . $res['id'] . "' AND `type` = 'm'" . (App::user()->rights >= 7 ? '' : " AND `close` != '1'") . " ORDER BY `time` DESC");
        ...
        default:
            ...
            */
            ...
            if ($total > 0) {
                $req = App::db()->query("SELECT * FROM `" . TP . "forum__`
                    ...
                    AND (`" . TP . "forum__rdm`.`topic_id` IS NULL
                    OR `" . TP . "forum__`.`time` > `" . TP . "forum__rdm`.`time`)
                    ORDER BY `" . TP . "forum__`.`time` DESC
                    " . App::db()->pagination()
                );
                for ($i = 0; $res = $req->fetch(); ++$i) {
                    if ($res['close']) {
                    ...
                    $q3 = App::db()->query("SELECT `id`, `refid`, `text` FROM `" . TP . "forum__` WHERE `type` = 'r' AND `id` = '" . $res['refid'] . "' LIMIT 1");
                    $razd = $q3->fetch();
                    $q4 = App::db()->query("SELECT `id`, `text` FROM `" . TP . "forum__` WHERE `type`='f' AND `id` = '" . $razd['refid'] . "' LIMIT 1");
                    $frm = $q4->fetch();
                    $colmes = App::db()->query("SELECT `from`, `time` FROM `" . TP . "forum__` WHERE `refid` = '" . $res['id'] . "' AND `type` = 'm'" . (App::user()->rights >= 7 ? '' : " AND `close` != '1'") . " ORDER BY `time` DESC");
                    $colmes1 = $colmes->rowCount();
                    $cpg = ceil($colmes1 / App::user()->settings['page_size']);
                    $nick = $colmes->fetch();
                    // Значки
    ...
    */
    ...
    $req = App::db()->query("SELECT * FROM `" . TP . "forum__` WHERE `type` = 't' AND `close` != '1' ORDER BY `time` DESC LIMIT 10");
    if ($req->rowCount()) {
        for ($i = 0; $res = $req->fetch(); ++$i) {
            $q3 = App::db()->query("SELECT `id`, `refid`, `text` FROM `" . TP . "forum__` WHERE type='r' AND id='" . $res['refid'] . "' LIMIT 1");
            $razd = $q3->fetch();
            $q4 = App::db()->query("SELECT `id`, `refid`, `text` FROM `" . TP . "forum__` WHERE type='f' AND id='" . $razd['refid'] . "' LIMIT 1");
            $frm = $q4->fetch();
            $nikuser = App::db()->query("SELECT `from`, `time` FROM `" . TP . "forum__` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" . $res['id'] . "'ORDER BY `time` DESC");

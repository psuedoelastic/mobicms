...
$req = App::db()->query("SELECT * FROM `download__category` WHERE `id` = '" . App::vars()->id . "' LIMIT 1");
$res = $req->fetch();
                ...
                if ($error) {
                    ...
                    if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "$load_cat/$fname")) == true) {
                        ...
                        if (!$set_down['mod'] || App::user()->rights > 6 || App::user()->rights == 4) {
                            ...
                            $i = 0;
                            while ($dirid != '0' && $dirid != "") {
                                $res_down = App::db()->query("SELECT `refid` FROM `download__category` WHERE `id` = '$dirid' LIMIT 1")->fetch();
                                if ($i)
                                    $sql .= ' OR ';
                                $sql .= '`id` = \'' . $dirid . '\'';

...
*/
if (App::user()->id) {
    $bookmark = App::db()->query("SELECT COUNT(*) FROM `download__bookmark` WHERE `file_id` = " . App::vars()->id . "  AND `user_id` = " . App::user()->id)->fetchColumn();
    if (isset($_GET['addBookmark']) && !$bookmark) {
        App::db()->exec("INSERT INTO `download__bookmark` SET `file_id`='" . App::vars()->id . "', `user_id`=" . App::user()->id);
        $bookmark = 1;
    } elseif (isset($_GET['delBookmark']) && $bookmark) {
        App::db()->exec("DELETE FROM `download__bookmark` WHERE `file_id`='" . App::vars()->id . "' AND `user_id`=" . App::user()->id);
        $bookmark = 0;
    }
...
if ((isset($_GET['plus']) || isset($_GET['minus'])) && !isset($_SESSION['rate_file_' . App::vars()->id]) && App::user()->id) {
    ...
    App::db()->exec("UPDATE `download__files` SET `rate`='" . $file_rate[0] . '|' . $file_rate[1] . "' WHERE `id`=" . App::vars()->id);
...
-----------------------------------------------------------------
*/
$req_file_more = App::db()->query("SELECT * FROM `download__more` WHERE `refid` = " . App::vars()->id . " ORDER BY `time` ASC");

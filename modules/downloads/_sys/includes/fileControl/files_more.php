...
if ($edit) {
    ...
    */
    ...
    $req_file_more = App::db()->query("SELECT `rus_name` FROM `download__more` WHERE `id` = '$edit' LIMIT 1");
    ...
    -----------------------------------------------------------------
    */
    $req_file_more = App::db()->query("SELECT `name` FROM `download__more` WHERE `id` = '$del'");
    if ($req_file_more->rowCount() && isset($_GET['yes'])) {
        $res_file_more = $req_file_more->fetch();
        if (is_file($res_down['dir'] . '/' . $res_file_more['name']))
            unlink($res_down['dir'] . '/' . $res_file_more['name']);
        App::db()->exec("DELETE FROM `download__more` WHERE `id` = '$del' LIMIT 1");
    ...
    */
    ...
    -----------------------------------------------------------------
    */
    $req_file_more = App::db()->query("SELECT * FROM `download__more` WHERE `refid` = " . App::vars()->id);

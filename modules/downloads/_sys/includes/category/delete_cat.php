...
if (App::user()->rights == 4 || App::user()->rights >= 6) {
    ...
    $req = App::db()->query("SELECT * FROM `download__category` WHERE `id` = " . $id);
    ...
    $res = $req->fetch();
    if (isset($_GET['yes'])) {
        $req_down = App::db()->query("SELECT * FROM `download__files` WHERE `refid` = " . $id);
        while ($res_down = $req_down->fetch()) {
            ...
            $req_file_more = App::db()->query("SELECT * FROM `download__more` WHERE `refid` = " . $res_down['id']);
            ...
            App::db()->exec("DELETE FROM `download__more` WHERE `refid` = " . $res_down['id']);
            App::db()->exec("DELETE FROM `download__comments` WHERE `sub_id` = " . $res_down['id']);
            App::db()->exec("DELETE FROM `download__bookmark` WHERE `file_id` = " . $res_down['id']);
        }
        App::db()->exec("DELETE FROM `download__files` WHERE `refid` = " . $id);
        App::db()->exec("DELETE FROM `download__category` WHERE `id` = " . $id);

        App::db()->query("OPTIMIZE TABLE `download__bookmark`, `download__files`, `download__comments`, `download__more`, `download__category`");

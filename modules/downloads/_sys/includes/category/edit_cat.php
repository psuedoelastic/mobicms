...
if (App::user()->rights == 4 || App::user()->rights >= 6) {
    ...
    */
    if (isset($_GET['up']) || isset($_GET['down'])) {
        ...
        $req_two = App::db()->query("SELECT * FROM `download__category` WHERE `refid` = '" . $res['refid'] . "' AND `sort` $val '" . $res['sort'] . "' ORDER BY `sort` $order LIMIT 1");
        if ($req_two->rowCount()) {
            $res_two = $req_two->fetch();
            App::db()->exec("UPDATE `download__category` SET `sort` = '" . $res_two['sort'] . "' WHERE `id` = '" . $id . "' LIMIT 1");
            App::db()->exec("UPDATE `download__category` SET `sort` = '" . $res['sort'] . "' WHERE `id` = '" . $res_two['id'] . "' LIMIT 1");
        }

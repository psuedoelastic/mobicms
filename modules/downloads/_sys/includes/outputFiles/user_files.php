...
$total = App::db()->query("SELECT COUNT(*) FROM `download__files` WHERE `type` = '2'  AND `user_id` = " . Mobi::$USER)->fetchColumn();
/*
-----------------------------------------------------------------
Навигация
-----------------------------------------------------------------
...
$i = 0;
if ($total) {
    $req_down = App::db()->query("SELECT * FROM `download__files` WHERE `type` = '2'  AND `user_id` = " . Mobi::$USER . " ORDER BY `time` DESC " . App::db()->pagination());

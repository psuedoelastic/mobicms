    ...
    switch ($do) {
        case 'clean':
            /*
               ...
               -----------------------------------------------------------------
               */
            $query = App::db()->query("SELECT `id`, `dir`, `name`, `type` FROM `download__files`");
            while ($result = $query->fetch()) {
                if (!file_exists($result['dir'] . '/' . $result['name'])) {
                    $req = App::db()->query("SELECT `id` FROM `download__more` WHERE `refid` = '" . $result['id'] . "'");
                    while ($res = $req->fetch()) {
                        @unlink($result['dir'] . '/' . $res['name']);
                    }
                    App::db()->exec("DELETE FROM `download__bookmark` WHERE `file_id`='" . $result['id'] . "'");
                    App::db()->exec("DELETE FROM `download__more` WHERE `refid` = '" . $result['id'] . "'");
                    App::db()->exec("DELETE FROM `download__comments` WHERE `sub_id`='" . $result['id'] . "'");
                    App::db()->exec("DELETE FROM `download__files` WHERE `id` = '" . $result['id'] . "' LIMIT 1");
                }
            }

            $query = App::db()->query("SELECT `id`, `dir`, `name` FROM `download__category`");
            while ($result = $query->fetch()) {
                if (!file_exists($result['dir'])) {
                    $arrayClean = [];
                    $req = App::db()->query("SELECT `id` FROM `download__files` WHERE `refid` = '" . $result['id'] . "'");
                    while ($res = $req->fetch()) {
                        $arrayClean = $res['id'];
                    }
                    $idClean = implode(',', $arrayClean);
                    App::db()->exec("DELETE FROM `download__bookmark` WHERE `file_id` IN (" . $idClean . ")");
                    App::db()->exec("DELETE FROM `download__comments` WHERE `sub_id` IN (" . $idClean . ")");
                    App::db()->exec("DELETE FROM `download__more` WHERE `refid` IN (" . $idClean . ")");
                    App::db()->exec("DELETE FROM `download__files` WHERE `refid` = '" . $result['id'] . "'");
                    App::db()->exec("DELETE FROM `download__category` WHERE `id` = '" . $result['id'] . "'");
                }
            }
            ...
            App::db()->query("OPTIMIZE TABLE `download__bookmark`, `download__files`, `download__comments`,`download__more`");
        ...
        default:
            /*
               ...
               */
            if ($id) {
                $cat = App::db()->query("SELECT `dir`, `name`, `rus_name` FROM `download__category` WHERE	`id` = '" . $id . "' LIMIT 1");
            ...
            if (isset($_GET['yes'])) {
                ...
                $query = App::db()->query("SELECT `dir`, `name`, `id` FROM `download__files`");
                while ($result = $query->fetch()) {
                    $array_dowm[] = $result['dir'] . '/' . $result['name'];
                    $array_id[$result['dir'] . '/' . $result['name']] = $result['id'];
                }

                $queryCat = App::db()->query("SELECT `dir`, `id` FROM `download__category`");
                while ($resultCat = $queryCat->fetch()) {
                    $array_dowm[] = $resultCat['dir'];
                    $array_id[$resultCat['dir']] = $resultCat['id'];
                }

                $query_more = App::db()->query("SELECT `name` FROM `download__more`");
                ...
                if ($id) {
                    $dir_files = App::db()->query("SELECT COUNT(*) FROM `download__files` WHERE `type` = '2' AND `dir` LIKE '" . ($res_down_cat['dir'] . '/' . $res_down_cat['name']) . "%'")->fetchColumn();
                    App::db()->exec("UPDATE `download__files` SET `total` = '$dir_files' WHERE `id` = '" . $id . "'");
                } else {
                    $req_down = App::db()->query("SELECT `dir`, `name`, `id` FROM `download__files` WHERE `type` = 1");
                    while ($res_down = $req_down->fetch()) {
                        $dir_files = App::db()->query("SELECT COUNT(*) FROM `download__files` WHERE `type` = '2' AND `dir` LIKE '" . ($res_down['dir'] . '/' . $res_down['name']) . "%'")->fetchColumn();
                        App::db()->exec("UPDATE `download__files` SET `total` = '$dir_files' WHERE `id` = '" . $res_down['id'] . "'");
                    }
                }

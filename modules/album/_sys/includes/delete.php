...
if ($al && $user['id'] == App::user()->id || App::user()->rights >= 6) {
    $req_a = mysql_query("SELECT * FROM `" . TP . "album__cat` WHERE `id` = '$al' AND `user_id` = '" . $user['id'] . "' LIMIT 1");
    if (mysql_num_rows($req_a)) {
        $res_a = mysql_fetch_assoc($req_a);
        echo '<div class="phdr"><a href="' . $url . '?act=list&amp;user=' . $user['id'] . '"><b>' . __('photo_album') . '</b></a> | ' . __('delete') . '</div>';
        if (isset($_POST['submit'])) {
            $req = mysql_query("SELECT * FROM `" . TP . "album__files` WHERE `album_id` = '" . $res_a['id'] . "'");
            while ($res = mysql_fetch_assoc($req)) {
                ...
                mysql_query("DELETE FROM `" . TP . "album__downlosystem__advt` WHERE `file_id` = '" . $res['id'] . "'");
                // Удаляем записи из таблицы голосований
                mysql_query("DELETE FROM `" . TP . "album__votes` WHERE `file_id` = '" . $res['id'] . "'");
                // Удаляем комментарии
                mysql_query("DELETE FROM `" . TP . "album__comments` WHERE `sub_id` = '" . $res['id'] . "'");
            }
            // Удаляем записи из таблиц
            mysql_query("DELETE FROM `" . TP . "album__files` WHERE `album_id` = '" . $res_a['id'] . "'");
            mysql_query("DELETE FROM `" . TP . "album__cat` WHERE `id` = '" . $res_a['id'] . "'");
            mysql_query("OPTIMIZE TABLE `" . TP . "album__cat`, `" . TP . "album__downlosystem__advt`, `" . TP . "album__votes`, `" . TP . "album__files`, `" . TP . "album__comments`");
